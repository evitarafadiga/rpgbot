...
bot.get_updates(fail_silently: true) do |message|
  ...
  message.reply do |reply|
    ...
    when /minhaclasse/i
      opclasses = ['Hey, Misdreavus! Temos uma bruxa presente!!!','Chico Xavier te batizou como MEDIUM.','Venha, Impidimp: leve este demônio, cria sua contigo!','Você é bruxa, eu sinto nas minhas entranhas!', 'Você cheira a demônio de longe...', 'Você não... Você é medium', 'Mais um medium...', 'Queimem a amaldiçoada bruxa!', 'Você é travesso igual a um demônio...']
  ...
  'https://play.pokemonshowdown.com/sprites/bw/drampa.png','https://play.pokemonshowdown.com/sprites/bw/oranguru.png','https://play.pokemonshowdown.com/sprites/bw/bouffalant.png','https://play.pokemonshowdown.com/sprites/bw/miltank.png','https://play.pokemonshowdown.com/sprites/bw/tauros.png','https://play.pokemonshowdown.com/sprites/bw/kangaskhan.png','https://play.pokemonshowdown.com/sprites/bw/typenull.png','https://play.pokemonshowdown.com/sprites/bw/meloetta.png']
    ...
    when /roletarussa/i
      ciladas = ['Eu decido, sim... TE EXECUTAR!', 'Eu realmente vou. Ah se vou... TE EXECUTAR!!!', 'Você vai passar batido dessa vez.', 'Por enquanto a sua pele está salva...', 'Você encontrou o tesouro!', 'Parabéns pela sorte de vencer essa rodada.', 'Me diga quem vai morrer no seu lugar...']
    ...
    when /greets/i
      reply.text = ["Olá, #{message.from.first_name}. Já anotei sim, seu nome no livro dos condenados...","Olá, #{message.from.first_name}. Reservei um lugarzinho especial aqui embaixo..." ]
    else
      reply.text = ["Você deve estar muito chapado para achar que eu sei o que é #{command.inspect}"," Eu não sei!","Eu não entendi","Não quero entender...","Eu não entendi o que raios #{command.inspect} significa."]
    end
